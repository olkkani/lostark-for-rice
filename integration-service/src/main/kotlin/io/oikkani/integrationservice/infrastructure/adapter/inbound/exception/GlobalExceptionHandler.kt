...
import io.oikkani.integrationservice.infrastructure.adapter.outbound.client.processor.ProcessorClientException
...
class GlobalExceptionHandler {
     ...
     * 에러 코드별 권장 대응 방안
     */
    private fun getRecommendedAction(errorCode: io.oikkani.integrationservice.infrastructure.adapter.outbound.client.processor.ProcessorErrorCode): String {
        return when (errorCode) {
            io.oikkani.integrationservice.infrastructure.adapter.outbound.client.processor.ProcessorErrorCode.NETWORK_ERROR ->
                "네트워크 연결 상태를 확인하고, 프로세서 서버 상태를 점검하세요."
            io.oikkani.integrationservice.infrastructure.adapter.outbound.client.processor.ProcessorErrorCode.TIMEOUT_ERROR ->
                "타임아웃 설정을 검토하고, 프로세서 서버 응답 시간을 모니터링하세요."
            io.oikkani.integrationservice.infrastructure.adapter.outbound.client.processor.ProcessorErrorCode.CLIENT_ERROR_4XX ->
                "요청 데이터나 인증 정보를 확인하세요. 재시도하지 않습니다."
            io.oikkani.integrationservice.infrastructure.adapter.outbound.client.processor.ProcessorErrorCode.SERVER_ERROR_5XX ->
                "프로세서 서버 상태를 확인하세요. 자동 재시도 후에도 실패했습니다."
            io.oikkani.integrationservice.infrastructure.adapter.outbound.client.processor.ProcessorErrorCode.RETRY_EXHAUSTED ->
                "모든 재시도가 실패했습니다. 서버 상태와 네트워크를 종합적으로 점검하세요."
            io.oikkani.integrationservice.infrastructure.adapter.outbound.client.processor.ProcessorErrorCode.UNKNOWN_ERROR ->
